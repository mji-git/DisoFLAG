...
def FLAG_model_running(input_data_file, output_file_name, embedding_file, output_type):
	...
	for t_batch in test_batches:   #一个batch
		...
		sys.stdout.flush()
		t_input_featues = t.tensor(np.array(t_batch.seq_T5_feature)).to(device)
		# seq_mask
		one_seq_mask = t.tensor(np.array(t_batch.seq_mask), dtype=t.float32).to(device)
		...
		IDR_probs.append(one_IDR_probs.detach().cpu().numpy())
		PB_probs.append(one_PB_logits.detach().cpu().numpy())
		DB_probs.append(one_DB_logits.detach().cpu().numpy())
		RB_probs.append(one_RB_logits.detach().cpu().numpy())
		IB_probs.append(one_IB_logits.detach().cpu().numpy())
		LB_probs.append(one_LB_logits.detach().cpu().numpy())
		Link_probs.append(one_Link_logits.detach().cpu().numpy())
